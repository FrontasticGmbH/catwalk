...
const autoGeneratedFileMessage = messageToTest + "\n    It should only be created in production builds "
    + "and not commited to your repository after a local production build."
...
function getCustomerOverrides () {
    ...
    if (data) {
        ...
        return [...data.matchAll(/(ComponentInjector.set\(\s*('|`|")[A-Za-z0-9]+('|`|")\s*,)/g)]
...
function getInjectorReturnExports (filePath) {
    ...
    if (data) {
        ...
        return [...data.matchAll(/(export \s*(default)?\s*ComponentInjector.return\(\s*('|`|")[A-Za-z0-9]+('|`|")\s*,)/g)]
            .map(res => res[0].match(/('|`|")[A-Za-z0-9]+('|`|")/)[0])
            .map(res => ({
 ...
 * @param {string} overwritePathName - The file name of the overridden file
 */
function generateReplacedComponentFileContents (replacedComponentString, overwritePathName) {
    ...
    const relativePathName = path.relative(path.dirname(overwritePathName), path.join(paths.catwalk, 'src/js/app/injector'))

    return `/*
    ${autoGeneratedFileMessage}
*/
// @ts-ignore
import ComponentInjector from './${relativePathName}'
 ...
 */
function overwriteComponent (replacedComponentString, overwritePathName) {
    const fileContents = generateReplacedComponentFileContents(replacedComponentString, overwritePathName)
 ...
 */
function handleComponentOverwrites (injectionReplacableComponents, componentsToExclude) {
    injectionReplacableComponents.forEach(injectionReplacableComponent => {
        ...
        if (exclIndex !== -1) {
            if (injectionReplacableComponent.originalContents.indexOf(messageToTest) === -1) {
                ...
                overwriteComponent(componentsToExclude[exclIndex].componentString, componentsToExclude[exclIndex].location)
 ...
 */
function overwriteInjectionReplacedComponents (PRODUCTION) {
    ...
    if (PRODUCTION) {
        ...
        handleComponentOverwrites(injectionReplacableComponents, componentsToExclude)
    }
    else {
