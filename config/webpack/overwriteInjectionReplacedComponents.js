...
const autoGeneratedFileMessage = "This file in autogenerated due to the component's override in injection.js.\n\tIt should only be "
    + "created in production builds and not commited to your repository after a local build."
...
function handleComponentOverwrites (injectionReplacableComponents, componentsToExclude, injectorAlias) {
    componentsToExclude.forEach(componentToExclude => {
        var exclIndex = injectionReplacableComponents.findIndex(repl => repl.componentString === componentToExclude.componentString)
        if (exclIndex > -1) {
            overwriteComponent(componentToExclude.componentString, injectorAlias, injectionReplacableComponents[exclIndex].location)
        }
    })
}
